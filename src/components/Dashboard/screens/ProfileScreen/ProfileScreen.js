...
const ProfileScreen = () => {
  ...
  const handleSubmit = (async (e) => {
    ...
    try {
      const res = await apiUserUpdateProfile({ fullName: fullName, address: address, phone: phone,image:pic })
      if (res) {
        setName(res.data.data.profile.fullName)
        setEmail(res.data.data.email)
        setAddress(res.data.data.profile.address)
      ...
      <div>
        ...
        <Row className="profileContainer">
          <Col md={6}>
            ...
            <img style={{ width: "50%", marginLeft: "70px" }} src='/assets/images/person.png'></img>
            <label class="custom-file-upload">
              <input type="file" value={pic}  onSubmit={handleSubmit} onChange={handleFileChange}/>
               Upload
            </label>
